    ...
    def batch_generate(self, image_dir, mask_dir):
        ...
        if mask_dir is not None:
            for fname in rgb_filename_list:
                cls_filepath_list.append(os.path.join(mask_dir, fname.replace("image","labels").replace("jpg","png")))
...
class CITY_OSMLoader(DataLoader, ConfigurableMixin):
    def __init__(self, config):
        ConfigurableMixin.__init__(self, config)
        dataset = CITY_OSM(self.config.image_dir, self.config.mask_dir, self.config.transforms)
        ...
        super(CITY_OSMLoader, self).__init__(dataset,
