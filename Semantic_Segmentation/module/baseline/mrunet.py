...
class mrUNet(er.ERModule):
    # def __init__(self,channels : int,filters : int =32,n_classes : int =1)->None:
    def __init__(self, config):
        ...
        Returns - None
        """
        super(mrUNet, self).__init__(config)
        self.loss = SegmentationLoss(self.config.loss)
        self.alpha = 1.67
        self.filters = config.filters
        self.n_classes = config.classes
        self.multiresblock1 = Multiresblock(input_features=config.num_channels,corresponding_unet_filters=self.filters)
        ...
        self.conv_final = Conv2d_batchnorm(input_features=self.in_filters9,num_of_filters = self.n_classes,
    ...
    def forward(self, x, y=None):
        ...
        o = self.conv_final(x_multires9)

        if self.training:
            return self.loss(o, y['cls'])
        return o.softmax(dim=1)
...
#     model=mrUNet(channels=3,filters=64 ,nclasses=2)
#     """
#     Arguments:
#     channels - input image channels
#     filters - filters to begin with (Unet)
