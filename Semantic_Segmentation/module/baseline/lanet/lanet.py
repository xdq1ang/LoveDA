...
from module.baseline.lanet.FCN_16s import FCN_res50 as FCN
#from FCN_16s import FCN_res50 as FCN
import ever as er
...
class ASP(nn.Module):
    ...
    def forward(self, feats):
        ...
        for idx, stage in enumerate(self.stages):
            ...
            priors.append(F.interpolate(input=feats_avg, size=(h, w), mode='bilinear', align_corners=True))
...
class LANet(er.ERModule):
    # def __init__(self, in_channels=3, num_classes=7, pretrained=True):
    def __init__(self, config):
        super(LANet, self).__init__(config)
        self.loss = SegmentationLoss(self.config.loss)
        self.FCN = FCN(config.in_channels, config.classes, config.pretrained)
        ...
        self.classifier0 = nn.Conv2d(64, config.classes, kernel_size=1)
        self.classifier1 = nn.Conv2d(128, config.classes, kernel_size=1)
    ...
    def forward(self, x, y=None):
        ...
        low = F.interpolate(low, x_size[2:], mode='bilinear',align_corners=True)
        
        high = self.classifier1(x2)
        high = F.interpolate(high, x_size[2:], mode='bilinear',align_corners=True)
                
        # high-level and low-level features are auxiliary outputs. Recommended loss: main_loss + 0.3*aux_loss1 + 0.3*aux_loss2
        
        if self.training:
            return self.loss(high+low, y['cls'])
        
        return (high+low).softmax(dim=1)
